...
template<int coeffIdx>
void inline filter8_s16x8(const int16x8_t *s, const int32x4_t c,
                           int32x4_t &d0, int32x4_t &d1)
{
    if (coeffIdx == 1)
    {
        // { -1, 4, -10, 58, 17, -5, 1, 0 }
                          ...
                          intptr_t dstStride)
{
    ...
    if (width % 8 != 0)
    {
        ...
        if (width == 12)
        {
            /* 8-row primary loop + 4-row tail for first 8 columns of width-12. */
            int16x8_t in[15];
            load_s16x8xn<7>(s, srcStride, in);
            s += 7 * srcStride;
            ...
            {
                load_s16x8xn<8>(s, srcStride, in + 7);
                ...
                filter8_s16x8<coeffIdx>(in + 2, c, sum_lo[2], sum_hi[2]);
                filter8_s16x8<coeffIdx>(in + 3, c, sum_lo[3], sum_hi[3]);
                filter8_s16x8<coeffIdx>(in + 4, c, sum_lo[4], sum_hi[4]);
                filter8_s16x8<coeffIdx>(in + 5, c, sum_lo[5], sum_hi[5]);
                filter8_s16x8<coeffIdx>(in + 6, c, sum_lo[6], sum_hi[6]);
                filter8_s16x8<coeffIdx>(in + 7, c, sum_lo[7], sum_hi[7]);

                uint8x8_t sum_u8[8];
                for (int i = 0; i < 8; i++)
                {
                    int16x8_t t = vcombine_s16(vshrn_n_s32(sum_lo[i], shift),
                                               vshrn_n_s32(sum_hi[i], shift));
                    sum_u8[i] = vqmovun_s16(t);
                }
                store_u8x8xn<8>(d, dstStride, sum_u8);

                in[0] = in[8];  in[1] = in[9];  in[2] = in[10]; in[3] = in[11];
                in[4] = in[12]; in[5] = in[13]; in[6] = in[14];
            ...
            for (; row < height; row += 4)
            {
                ...
                in[4] = in[8]; in[5] = in[9]; in[6] = in[10];
        ...
        /* Remaining 4 columns (width==4 standalone, or width==12 cols 8..11). */
        int16x4_t in[15];
        load_s16x4xn<7>(s, srcStride, in);
        s += 7 * srcStride;
        ...
        for (; row + 7 < height; row += 8)
        {
            load_s16x4xn<8>(s, srcStride, in + 7);
            ...
            filter8_s16x4<coeffIdx>(in + 2, c, sum[2]);
            filter8_s16x4<coeffIdx>(in + 3, c, sum[3]);
            filter8_s16x4<coeffIdx>(in + 4, c, sum[4]);
            filter8_s16x4<coeffIdx>(in + 5, c, sum[5]);
            filter8_s16x4<coeffIdx>(in + 6, c, sum[6]);
            filter8_s16x4<coeffIdx>(in + 7, c, sum[7]);

            uint8x8_t sum_u8[8];
            for (int i = 0; i < 8; i++)
            {
                int16x4_t t = vshrn_n_s32(sum[i], shift);
                sum_u8[i] = vqmovun_s16(vcombine_s16(t, vdup_n_s16(0)));
            }
            store_u8x4xn<8>(d, dstStride, sum_u8);
    ...
    else
    {
        for (int col = 0; col < width; col += 8)
        {
            ...
            /* Window: preamble (7 rows) + up to 8 new rows = 15 slots. */
            int16x8_t in[15];
            load_s16x8xn<7>(s, srcStride, in);
            s += 7 * srcStride;

            /* 8-row primary loop: 8 independent accumulator chains expose
             * maximum ILP to the CPU's out-of-order engine, hiding the
             * MLA dependency-chain latency (~12 cycles critical path after
             * the A/B chain split above). */
            int row = 0;
            for (; row + 7 < height; row += 8)
            {
                load_s16x8xn<8>(s, srcStride, in + 7);
            ...
            /* 4-row tail: handles heights not divisible by 8 (e.g. 4, 12). */
            for (; row < height; row += 4)
            {
                load_s16x8xn<4>(s, srcStride, in + 7);
